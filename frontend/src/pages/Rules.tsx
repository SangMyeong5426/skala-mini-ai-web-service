 ...
 *
 * <b>부족한 정보를 채워 다시 판정받는 길은 아직 없다.</b> 이 주석은 예전에
 * "그 경로는 챗봇" 이라고 적어 두었는데 사실이 아니었다. 챗봇의 `RULE_CHECK` 은
 * `tripId`·`itemId` 를 일부러 안 보낸다 — 대화 이력을 저장하지 않기로 했기
 * 때문이다(02:154). 그래서 답은 대화창에만 남고 이 여행의 `item_rule_checks`
 * 도 검수 결과도 바뀌지 않는다.
 *
 * 그래서 여기는 <b>이름만 알려 준다</b> — 무엇이 부족한지. 값을 받는 입력칸은
 * 두지 않는다. 받아 봐야 보낼 곳이 없어서, 채워 넣은 사람에게 <b>보완했다는
 * 착각</b>만 남긴다. 재판정 연결은 03 의 S-08 에 보류로 적었다.
 */
