...
> 반영해 기존 업무 API 18개에 인증 API 4개를 더했고, 이후 일정·캘린더 5개와 3D 정리 3개를
> 추가해 **총 30개**를 구현했다. 현재 코드·시드의 반영 상태는
> [문서 지도](README.md#개정안-반영-상태)에 적는다.
...
**API 1~30과 기존 `/uploads/**` 파일 조회는 모두 인증 대상이다.** 아래 주요 오류에
...
| 4 | `PATCH` | `/api/trips/{tripId}` | 여행 수정 · **최종 저장**(`status`) | `200` | `400` `404` |
...
30개 JSON API와 별도로 **기존 사진 파일 조회 경로를 보호**하는 것이며 파일용 신규 API는 추가하지 않는다.
...
## AI 확장 지점 엔드포인트 (작업별 실제/Mock)

**AI-Ready 원칙 3 (Asynchronous Pipeline)을 구현한 형태다.** 실제/Mock 여부와 무관하게
같은 작업 생성·폴링 계약을 사용한다.
...
| --- | --- | --- | --- | --- |
| `PACKING_LIST` | UC-05 추가 준비물 추천 | `S-05` | `openai`면 실제 LLM | 추천 품질 검증 |
| `BAG_CHECK` | UC-04 사진 물품 인식 | `S-04` | `openai`면 실제 비전 모델 | 인식 품질 검증 |
| `WEIGHT_ESTIMATE` | UC-10 예상 무게 산정 | `S-06` `S-07` | **항상 Mock 품목 범위 + 서버 합산** | 품목 중량 DB + LLM 보정 |
| `RULE_CHECK` | UC-07 · UC-08 반입 규정 | `S-06` `S-08` `S-09` | 여행 연결은 실제 AI, 여행 없는 챗봇은 **Mock** | 규칙 엔진 판정은 공통 |
...
`RULE_CHECK` 챗봇은 `AI_PROVIDER=openai`에서도 Mock을 유지하며 S-09의 대표 질문 12개를 지원한다. 질문 문구의 공백·영문 대소문자는
...
**예산은 횟수가 아니라 시간이다.** 간격은 서버가 `pollAfterMs` 로 정하므로,
횟수로 정하면 실제 대기 시간이 서버 손에 있게 된다 — `pollAfterMs: 500` 에
60회면 30초다. 서버의 AI 호출 제한은 `app.ai.timeout-ms` 로 **60초**이고
재시도가 한 번 더 있어, 30초에서 접으면 **서버가 아직 일하는 중인데 화면이
먼저 포기한다.** 90초는 그 60초에 재시도·네트워크 여유를 더한 값이다.

프런트엔드는 `pollAfterMs` 를 계속 존중하되 **250ms 를 하한으로 둔다.**
서버가 `0` 을 보내면 예산이 다 되기 전까지 요청을 몰아치기 때문이다.
다음 대기가 예산을 넘기면 더 기다리지 않는다.

### 최종 저장 (S-06)

03:184 이 정한 흐름의 마지막 단계다 — *"검수 결과 확인·수정 → 최종 저장"*.
새 여행은 `POST /api/trips` 가 **`DRAFT`(작성 중)** 로 만들고 클라이언트가
`status` 를 정하지 않는다. 검수를 마치면 S-06 의 `최종 저장` 이
`PATCH /api/trips/{tripId}` 로 **`CONFIRMED`(진행 중)** 를 보낸다.
...
미채택 필수 후보가 있어도 저장을 막지 않는다(위 S-06 경고 규약).

## 주요 응답 예시

각 예시는 해당 동작 시점의 응답이다. 추천 작업의 최초 완료 응답은 채택 전이며,
아래 내 목록 조회·검수 예시는 사용자가 후보를 채택한 후의 상태를 보여준다.
두 조회는 **같은 tripId 12의 전체 내 목록 9개(완료 8·미완료 1)**를 사용한다.
...
상태다. 부분 집계 예시의 `customs`는 보조배터리 한 건만 보여준다. 현재 SQL 시드도
사진 인식 가위를 포함해 8개가 자동 완료된 상태다.
...
4. 한 작업의 완료 처리는 한 번만 한다. 같은 완료 처리 재시도·GET 폴링은 항목을 다시 생성하거나
   ...
   준비 상태를 덮어쓰지 않으면서 새로 인식된 다른 물품은 추가할 수 있다. 이름이 다르면 배열
   순서만으로 같은 물품이라 간주하지 않고 새 인식을 모두 저장한다.
...
| 수정 필드 전부 생략 | 아무 값과 사후 수정 표식도 바꾸지 않고 현재 결과를 반환한다 |
