...
> F-04~F-06을 적용했다. 사진 승인으로 내 목록을 만들고, 추가 추천은 별도 후보로
> 제시한다. 사용자가 채택한 후보만 미완료로 추가한다.


## Use-Case 다이어그램

> **아래 PNG·PUML·SVG는 개정 전 참고본이다.** UC-05의 명칭과 승인 책임은 아래 표가
> 최신 기준이며 이미지 재생성은 TBD다. 기능 ID와 Actor 관계는 유지한다.
...
사람 Actor는 **여행자 한 명**이다. 이번 데모에서 인증 흐름을 구현하지 않으므로
(로그인 화면이 없다, [`01-service-plan.md`](01-service-plan.md)) 인증 상태에 따른
...
| --- | --- | --- | --- | --- | --- |
| UC-01 | F-01 | ~~회원가입 및 로그인~~ | — | **범위 제외.** `users` 에 `password_hash` 자리는 두었지만 이번 데모에서 로그인 화면과 인증 흐름을 만들지 않는다. 시드 사용자 한 명으로 고정한다 | 제외 |
| UC-02 | F-02 | 여행 등록 | 여행자 | 추천과 수하물 판단에 필요한 여행 조건을 구조화한다 | Must |
| UC-03 | F-03 | 짐 사진 등록 | 여행자 | 물품 인식에 적합한 이미지를 안정적으로 수집한다 | Must |
| UC-04 | F-04 | **AI 사진 속 물품 인식** | 여행자 ↔ **AI 서비스** | 후보를 확인·승인하고 내 목록에 챙김 완료로 등록한다 | Must |
...
| 기본 흐름 | 1. AI 작업을 접수하고 작업 ID를 반환한다<br>2. 화면은 분석 중 상태를 표시하고 폴링한다<br>3. 물품 후보·수량·신뢰도와 OCR 결과를 만든다<br>4. 여행자가 동일 물품 후보를 연결하고 최종 수량을 확인한다<br>5. 이름·수량을 수정·승인한다<br>6. 서버가 기존 항목과 연결하거나 새 항목을 생성하고 완료 처리한다 |
| 대체·예외 흐름 | 낮은 신뢰도는 복수 후보를 제시한다. 보이지 않는 용량·정격·날 길이는 라벨 사진이나 직접 입력을 요청한다 |
| 사후 조건 | **사진 승인 요청이 성공하면 내 목록 등록과 완료 처리가 끝나 있다.** 추천 작업과 무관하다. 미승인 후보는 확정 준비 상태·무게·규정 결과에 반영하지 않는다 |
...
| 대체·예외 흐름 | 예보 범위 초과·날씨 조회 실패 시 대체 기준과 데이터 시점을 표시한다. 추천 실패 시 이미 등록된 내 목록을 유지하고 재시도·직접 추가를 제공한다. 미선택 후보는 내 목록에 넣지 않는다 |
...
| --- | --- |
| Actor | 여행자 |
| 사전 조건 | 여행이 있다. 내 목록은 사진 승인·추천 채택·직접 추가로 생성되며 처음에는 비어 있을 수 있다 |
...
| --- | --- |
| Actor | 여행자 ↔ AI 서비스 |
| 사전 조건 | 사진 승인 또는 직접 완료 확인으로 `PREPARED`가 된 내 목록 항목이 있다 |
...
| 대체·예외 흐름 | 미승인 인식 후보·미채택 추천·내 목록의 미완료 항목은 합계에서 제외한다. 사진에서 보이지 않아도 직접 챙김 완료를 확인한 항목은 포함한다. 사진만으로 식별이 어려우면 직접 확인·재촬영을 요청한다. 무게 정보가 없으면 제외 이유를 표시하고 한도 근접 시 실제 측정을 권고한다 |
...
| UC-04 물품 인식 | **UC-05**, UC-06, UC-07, UC-10 | 내 목록에 등록된 완료 물품과 승인 물품명·수량·라벨 정보 |
...
| 서버 서비스 계층 | 사진 승인 등록, 추천 채택·중복 방지, 완료율과 무게 합산 | 사용자를 대신해 추천 채택 |
| 여행자 | 인식 결과 승인, 추천 선택·승인, 실제 준비 여부와 최종 반입 확인 | 서비스 결과로 출발 당일 관계기관 판단 대체 |
...
### ② 사진 승인 → 내 목록 → 별도 추천 → 사용자 채택

```text
사진 등록(UC-03) → 인식 결과 승인·완료 등록(UC-04)
  → 별도 추가 추천(UC-05) → 필요한 후보만 미완료로 채택 → 실제 준비 관리(UC-06)
```

기존 사진 우선 결정은 유지하되, 2026-09-03 개정안에 따라 **내 목록 등록을 추천 작업에서
분리**한다. 사진 승인과 추천 채택은 다른 승인이다. 사진에서 찾지 못한 항목을 누락으로
단정하지 않으며, 승인한 사진 물품을 별도 추가 물품 영역에만 남겨 두지 않는다.

기능 ID는 식별자이며 실행 순서를 강제하지 않는다. 사진 없이 시작하면 추천 후보를
검토·채택하거나 직접 추가할 수 있다. 이미 등록된 동일 물품의 부족 수량 추천은 **TBD**다.

### ③ 원본·개정안·데모 범위의 구분

[개인 개정안](https://app.notion.com/p/3d0c2ab24ce881d9b06cc065c47b1eb7)이 F-04~F-06 흐름의
기준이다. 팀 원본을 수정 완료했다고 가정하지 않는다. 개정안 상세 페르소나의 축약 흐름보다
1절·F-04~F-06의 명시적 승인 규칙을 따른다.

로그인 최종 정책과 실측값 입력은 이번 개정으로 구현 범위에 추가하지 않는다.
인증은 데모 제외, 물품·전체 가방 실측값 저장은 **TBD**다(01·03).
