...
| ID | 기능 ID | Use-Case | Actor | 인증 필요 | 목적 | 우선순위 |
| --- | --- | --- | --- | --- | --- | --- |
| UC-01 | F-01 | 회원가입 및 로그인 | 여행자 | 불필요 | 개인 여행정보와 분석 기록을 안전하게 관리한다 | Must |
| UC-02 | F-02 | 여행 등록 | 여행자 | 불필요(임시 모드) | 추천과 수하물 판단에 필요한 여행 조건을 구조화한다 | Must |
| UC-03 | F-03 | 짐 사진 등록 | 여행자 | 필요 | 물품 인식에 적합한 이미지를 안정적으로 수집한다 | Must |
| UC-04 | F-04 | **AI 사진 속 물품 인식** | 여행자 ↔ **AI 서비스** | 필요 | 물품 후보와 수량을 구조화해 비교·무게 계산의 기초자료를 만든다 | Must |
| UC-05 | F-05 | **AI 맞춤 체크리스트 생성** | 여행자 ↔ **AI 서비스**, 날씨 서비스 | 필요 | 여행 조건과 예상 날씨에 맞는 준비물을 제안한다 | Must |
| UC-06 | F-06 + 흐름 7단계 | 체크리스트 관리 | 여행자 | 필요 | 체크리스트를 관리하고 승인 물품과 비교해 준비 상태를 확정한다 | Must |
| UC-07 | F-07 | 항공 반입 규정 확인 | 여행자 ↔ **AI 서비스**(부분), 공식 규정 데이터 출처 | 필요 | 공식 기준으로 물품별 기내·위탁 가능 여부를 안내한다 | Must |
| UC-08 | F-08 | **수하물 확인 챗봇** | 여행자 ↔ **AI 서비스**, 공식 규정 데이터 출처 | 불필요 | 복잡한 수하물 질문을 대화형으로 해결한다 | Should |
| UC-09 | F-09 | 여행 기록 조회 및 재사용 | 여행자 | 필요 | 반복 여행의 준비 시간을 줄인다 | Should |
| UC-10 | F-10 | **사진 기반 예상 무게 산정** | 여행자 ↔ **AI 서비스** | 필요 | 가방 총중량과 한도 초과 가능성을 추정한다 | Must·Beta |
...
| 사후 조건 | 승인된 물품만 UC-06·UC-07·UC-10에 반영할 수 있다 |
...
| 기본 흐름 | 1. 여행 정보와 예상 날씨를 불러온다<br>2. AI 작업을 접수하고 작업 ID를 반환한다<br>3. 여행 목적·기간·장소·계절에 맞는 준비물 후보를 만든다<br>4. 고정 필수 규칙으로 결과를 보강한다<br>5. 여행자가 추가·수정·삭제 후 확정한다 |
| 대체·예외 흐름 | 예보 범위 초과·날씨 조회 실패 시 계절 평균과 데이터 시점을 표시한다. AI 실패 시 기본 체크리스트를 제공한다 |
| 사후 조건 | **사진 등록보다 먼저** 여행별 기본 체크리스트가 저장된다 |
| 관련 API | `POST /api/ai-jobs` (`job_type: PACKING_LIST`) → `202`<br>`GET /api/ai-jobs/{id}` → `200` |
| 관련 화면 | `S-04` 체크리스트 |
...
| --- | --- |
| Actor | 여행자 |
| 사전 조건 | 체크리스트가 있다. 자동 비교에는 승인 물품(UC-04)이 추가로 필요하다 |
...
| 관련 화면 | `S-07` 검수 결과, `S-09` 반입 규정 상세 |
...
| UC-04 물품 인식 | UC-06, UC-07, UC-10 | 여행자가 승인한 물품명·수량·라벨 정보 |
...
팀 결정 [#14](https://github.com/SangMyeong5426/skala-mini-ai-web-service/pull/14)와
기능 정의 3절의 순서를 따른다.

```text
체크리스트 생성(UC-05) → 사진 등록(UC-03) → 물품 인식(UC-04) → 목록 비교(UC-06)
```

F-05 목적의 "이미지 인식을 통해 확인된 물품 체크리스트에 등록"은 체크리스트
생성의 선행 입력이 아니라 뒤 단계인 목록 비교로 해석한다. 준비물 비교는 독립
기능 ID를 만들지 않고 UC-06 체크리스트 관리에 포함하며, 사진에서 찾지 못한
항목은 자동 누락으로 판정하지 않는다.
