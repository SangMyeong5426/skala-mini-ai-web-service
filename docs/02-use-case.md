...
| 관련 API | `POST /api/ai-jobs` (`jobType: BAG_CHECK`) → `202`<br>`GET /api/ai-jobs/{id}` → `200` |
| 관련 화면 | `S-04` 인식 결과·승인 |
...
| 관련 API | `POST /api/ai-jobs` (`jobType: PACKING_LIST`) → `202`<br>`GET /api/ai-jobs/{id}` → `200` |
| 관련 화면 | `S-05` 체크리스트 |
...
| 관련 API | `POST /api/ai-jobs` (`jobType: RULE_CHECK`) → `202`<br>`GET /api/ai-jobs/{id}` → `200` |
| 관련 화면 | `S-09` 챗봇 |
...
| 관련 API | `POST /api/ai-jobs` (`jobType: WEIGHT_ESTIMATE`) → `202`<br>`GET /api/ai-jobs/{id}` → `200` |
