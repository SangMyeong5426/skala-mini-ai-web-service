...
> **2026-09-03 최종 로그인 결정 반영**
> 로그인한 사용자가 사진으로 챙긴 물품을 먼저 등록하고, 추가 추천은 직접 선택한다.
...
이번 로그인 결정과 아래 규약을 저장소의 개정 기준으로 사용한다. Notion 원문을 수정한 것은 아니다.
...
- **설계와 구현:** AI는 전부 Mock이다. 이 문서·01~07·다이어그램의 인증 설계 반영이 현재 코드의 로그인 구현 완료를 의미하지 않는다.
...
**회원가입(최초 1회) → 로그인 → 여행 등록 → 사진 업로드 → 물품 인식·확인 → 내 체크리스트에 ‘챙김 완료’로 등록 → 아래에 별도 AI 추천 표시 → 필요한 추천 선택·승인 → 내 체크리스트에 ‘미완료’로 추가**

### 두 목록의 차이

| 구분 | 내 체크리스트 | AI 추천 목록 |
| --- | --- | --- |
| 의미 | 사용자가 챙긴 물품과 챙기기로 선택한 물품 | 여행 조건에 따라 추가로 챙길지 검토할 후보 |
| 초기 내용 | 사진에서 인식하고 사용자가 확인한 물품 | 이미 등록된 물품을 제외한 추가 추천 |
| 표시 위치 | 화면 위쪽 | 내 체크리스트 아래쪽 |
| 등록 시점 | 사진 인식 결과 확인 또는 추천 선택·승인 시 | 추천 생성 시 후보로 표시 |
| 준비 완료율 | 포함 | 승인 전에는 제외 |

**‘추천 승인’은 챙길 목록에 넣겠다는 뜻이며, 실제로 챙겼다는 뜻은 아니다.** 사진에서 확인한 물품은 챙김 완료로 등록하고, 추천에서 선택한 물품은 미완료로 등록한다.

### 사용 예시

사진에서 **충전기·상의·보조배터리**를 인식하고 사용자가 물품명·수량을 확인했다.
...
2. AI는 물품 후보·수량·신뢰도를 보여준다. 사용자는 기존 기능 정의대로 오인식·수량을 확인하고 수정·승인한다.
3. 승인된 사진 물품으로 내 체크리스트를 생성한다. 이미 목록이 있으면 기존 항목과 연결하거나 없는 항목을 추가한다.
4. 사진에서 확인된 물품은 ‘챙김 완료’로 표시한다. 체크리스트가 없다는 이유로 등록에서 빠지면 안 된다.
5. AI는 이미 챙긴 물품을 중복 추천하지 않고, 여행 조건에 따른 추가 후보를 만든다.
...
> 사진 인식 결과 확인과 추천 선택·승인은 서로 다른 동작이다. 전자는 ‘사진 속 물품이 맞다’는 확인이고, 후자는 ‘앞으로 이 물품도 챙기겠다’는 선택이다. 사진에서 찾지 못했다는 이유만으로 실제 누락을 확정하지 않는다.
>
...
flowchart TD
  ...
  B --> C["사용자가 물품명·수량 확인 및 승인"]
  C --> D["내 체크리스트 생성<br>인식 물품은 챙김 완료"]
...
**서비스 목적:** 로그인한 사용자가 촬영한 짐 사진에서 물품을 인식하고, 확인된 물품을 내 체크리스트에 챙김 완료로 등록한다. 여행 조건에 맞는 추가 준비물은 아래의 별도 AI 추천 목록에 제시하고, 사용자가 필요한 항목만 선택·승인해 자신의 체크리스트에 추가한다. 이후 준비 상태, 반입 규정, 예상 무게를 확인한다.

**사용자 역할:** 사진 인식 결과의 물품명·수량을 확인·수정하고, 추가 추천 중 필요한 항목을 선택한다. 추천을 목록에 추가하는 동작과 실제 챙김 완료 처리를 구분하며, 보이지 않는 속성과 실측값을 보완하고 최종 상태를 확정한다.

**핵심 원칙:** 사진에서 확인한 물품, AI 추천 후보, 사용자가 채택한 준비물을 구분한다. 불확실성을 공개하고, 사진 미인식 항목을 실제 누락으로 단정하지 않는다. 무게는 범위와 신뢰도를 함께 제공한다.

### 페르소나의 주요 이용 흐름

**일반 항공 여행객:** 회원가입·로그인 → 여행 등록 → 짐 사진 등록 → 인식 물품·수량 확인 → 챙긴 물품으로 내 체크리스트 생성 → 아래쪽 AI 추천에서 필요한 항목 선택·승인 → 추가 준비 및 완료 체크 → 반입 규정 확인 → 최종 저장.

**장기 여행자:** 회원가입·로그인 → 장기 일정 등록 → 여러 장의 짐 사진 등록 → 물품·수량 확인 → 챙긴 물품으로 내 체크리스트 생성 → 장기 체류에 필요한 추가 추천 검토·승인 → 추가 준비 → 가방·고밀도 물품 정보 입력 → 예상 무게 확인 → 가방 재구성.
...
`회원가입·로그인 → 여행 등록 → 사진 승인·챙김 완료 등록 → 별도 추천 선택·미완료 추가 → 실제 준비 확인 → 반입 규정 확인 → 최종 저장`
...
`로그인 → 가족 구성원 등록(확장) → 연령·개인 특이사항 입력 → 구성원별 체크리스트 생성 → 담당자 지정 → 사진 등록 → 필수품·수량 검수 → 최종 확인`
...
`회원가입·로그인 → 장기 일정 등록 → 여러 사진의 물품·수량 승인·완료 등록 → 별도 추천 선택·미완료 추가 → 실제 준비·가방 정보 확인 → 예상 무게 확인 → 가방 재구성`
...
### 3단계 · 인식 확인과 내 체크리스트 생성

- **사용자:** 인식된 물품명·수량을 확인하고 오인식을 수정·삭제·승인한다.
- **시스템:** 승인한 물품을 내 체크리스트에 챙김 완료로 등록한다. 목록이 없으면 생성하고, 기존 항목과 연결할 수 있으면 중복 없이 반영한다.
- **확인:** 낮은 신뢰도의 후보와 미식별 항목은 확인 전 최종 준비 상태에 반영하지 않는다. 배터리 정격·용량 등 보이지 않는 속성은 별도로 입력받는다.
...
- **시스템:** 실제 챙김이 확인된 승인 물품을 기준으로 무게 범위·기여도·신뢰도·제외 항목을 표시한다. 반입 규정은 공식 기준과 입력 조건을 비교하여 근거·확인일·필요 행동을 안내한다.
...
**목적:** 사진 속 물품을 인식하고, 사용자가 확인한 물품을 내 체크리스트에 챙김 완료로 등록한다.
...
- 사용자가 추가·삭제·수정·승인할 수 있게 한다.
- 승인 물품으로 내 체크리스트를 생성하거나 기존 목록에 연결·추가하고 챙김 완료로 표시한다.
- 추천 생성에는 승인된 물품 목록을 전달하여 중복 추천을 막는다.

**출력:** 확인·수정 필요 후보, 미식별 항목, 승인 물품, 승인 물품이 반영된 내 체크리스트.

**예외·검증:** 낮은 신뢰도는 단정하지 않는다. 미승인 결과는 최종 준비 상태·무게·규정의 확정 결과에 반영하지 않는다. 사진 인식 승인과 후속 추천 승인을 혼동하지 않는다.
...
**입력:** 항목명, 카테고리, 수량, 완료 여부, 메모, 사진 인식 승인 결과, 사용자가 선택·승인한 추천 항목.
...
| 상태 | 의미 | 다음 행동 |
| --- | --- | --- |
| 사진 인식·확인 대기 | 아직 사용자가 확인하지 않은 물품 후보 | 물품명·수량 수정 후 승인 |
| 내 체크리스트·완료 | 사진에서 확인하거나 사용자가 실제로 챙겼다고 확인한 물품 | 필요 시 내용·상태 수정 |
...
- **기존 목록에 없던 인식 물품:** 사용자가 인식 결과를 승인하면 내 목록에 등록한다. 승인 후에도 별도 ‘추가 물품’에만 남겨 두지 않는다.

### 역할

- **이미지 AI·OCR:** 사진 속 물품 후보·수량·라벨정보·신뢰도를 제시한다.
- **생성형 AI:** 이미 확인된 물품을 고려하여 추가 후보와 이유를 추천한다. 사용자의 추천 채택을 대신 결정하지 않는다.
- **시스템:** 로그인·세션·소유권 검증, 인식 승인 물품의 등록, 추천 후보와 내 목록의 분리, 추천 승인 후 미완료 등록, 중복 방지와 완료율 계산을 담당한다.
- **규칙 엔진:** 입력 조건과 공식 규정을 비교한다. 정보가 부족하면 확정 판정을 보류한다.
- **사용자:** 인식 결과를 확인하고, 필요한 추천을 선택하고, 실제 준비 상태와 실측값을 확정한다.
...
현재 가방의 예상 무게는 **물품명·수량이 확인되고 실제 챙김이 확인된 물품**을 대상으로 계산한다. 사진에서 확인한 물품뿐 아니라 사용자가 직접 챙김 완료를 확인한 물품도 같은 기준으로 취급한다.

- 인식 승인 전 후보와 미식별 항목은 계산에서 제외하고 이유를 표시한다.
- AI 추천 후보는 계산에서 제외한다.
...
1. 빈 체크리스트에서 사진을 올리고 충전기·상의·보조배터리를 승인하면 내 목록에 세 물품이 모두 챙김 완료로 등록된다.
...
7. 인식 결과의 이름·수량 수정과 미승인 후보 제외가 목록·비교·무게에 일관되게 반영된다.
...
| 장기 여행자 · 주요 이용 흐름 | 사진·수량 확인 후 내 목록 생성, 추가 추천은 선택 승인 후 반영한다고 명시 |
| 전체 서비스 흐름 | 기존 ‘체크리스트 생성’을 인식 물품 등록·추천 표시·추천 승인 단계로 구체화 |
| F-04 · 사진 인식 | 승인된 인식 물품이 내 체크리스트에 챙김 완료로 등록되는 결과를 명시 |
...
| 공통 품질 · 사용자 승인 | 인식 승인과 추천 승인을 각각 검증 |

## 8. 적용 범위와 미정 정책

- **작성 근거:** 사용자가 붙여넣은 개인 Notion 개정안과 이번 로그인 최종 결정. 개정안은 「기능 정의」 PDF와 2026-09-03의 6단계 체크리스트 흐름을 근거로 작성됐다고 명시한다.
...
- **문서·코드 반영:** 이 기능정의서와 01~07·다이어그램은 로그인 포함 설계로 갱신한다. 실제 인증 코드·DB 마이그레이션·Mock 반영 및 E2E는 별도 작업이다.
...
- **기존 리뷰 반영 유지:** 완료율은 내 목록 항목 수 기준 소수 셋째 자리로 반올림하며, 미채택 필수 후보 경고는 100%에서도 유지한다. S-06 사진 확인 필요는 S-04 연결·승인으로 해소한다(03·06).
