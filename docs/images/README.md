...
| 파일 | 반영 내용 |
| --- | --- |
| `02-usecase.*` | UC-01 가입·로그인 복원, UC-02~10 공통 로그인 사전 조건. 사진 승인 등록·추천 선택 채택 책임 |
| `03-userflow.*` | S-00 가입·로그인 → 사진 승인 즉시 완료 등록 → 별도 추천 → 선택분 미완료 추가 → 실제 완료 확인. 필수 후보 경고·S-04 재확인·재사용·챗봇 경로 포함 |
| `04-architecture.*` | Spring Security·서버 세션·CSRF·본인 소유권과 Service의 승인·채택·계산 책임. AI 작업의 비동기 폴링 유지 |
