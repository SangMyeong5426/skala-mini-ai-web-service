...
**4종의 원본·PNG·SVG에 로그인 필수와 개정 흐름을 반영했다.** 아래 그림은 기능 설계이며,
현재 코드의 구현 완료를 뜻하지 않는다. 구현·시드의 후속 작업은 [반영 상태](../README.md#개정안-반영-상태)를 따른다.

| 파일 | 반영 내용 |
| --- | --- |
| `02-usecase.*` | UC-01 가입·로그인 복원, UC-02~10 공통 로그인 사전 조건. 사진 자동 등록·추천 선택 채택 책임 |
| `03-userflow.*` | S-00 가입·로그인 → 사진 인식 즉시 자동 완료 등록 → 별도 추천 → 선택분 미완료 추가 → 실제 완료 확인. 필수 후보 경고·S-04 재확인·재사용·챗봇 경로 포함 |
| `04-architecture.*` | Spring Security·서버 세션·CSRF·본인 소유권과 Service의 자동 등록·추천 채택·계산 책임. AI 작업의 비동기 폴링 유지 |
| `05-erd.*` | 기존 10개 테이블·관계 유지, 목표 users.login_id 추가(SQL 미반영). 사용자 소유권과 후보 JSON의 저장 경계 표시 |
