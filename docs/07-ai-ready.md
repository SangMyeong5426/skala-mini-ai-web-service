...
```json
{
  ...
  "properties": {
    ...
    "alreadyPacked": {
      "type": "array",
      "maxItems": 100,
      "description": "해당 여행의 PREPARED 내 목록 항목. 사진 승인·직접 완료 모두 포함하며 없으면 빈 배열",
      "items": {
        "type": "object",
        "properties": {
          "name": {
...
**작업 접수 전:** alreadyPacked를 해당 여행의 현재 PREPARED 항목명·수량과 대조한다.
불일치하면 작업을 만들지 않고 `409`로 새로고침을 요청한다.
...
이번 3일 데모는 AI 전부 Mock이며 Playground·실제 API 호출을 검증 관문으로 두지 않는다.
실제 AI 결합 단계에서 프롬프트와 모델용 파생 스키마를 검증한다. 그때도 등록·채택·완료율·
무게 합산은 서버가 담당하고 같은 수용 기준을 통과해야 한다.
