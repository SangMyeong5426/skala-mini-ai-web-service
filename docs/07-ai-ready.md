...
- **엄격도 순서가 두 곳에서 다르다.** `RuleEngine`은 이 문서의 순서
  (`CHECKED_FORBIDDEN > RESTRICTED > CHECKED_OK > ASK_AIRLINE > CABIN_OK`)를 따르지만,
  `InspectionService.STRICTNESS`는 `ASK_AIRLINE`을 `RESTRICTED`·`CHECKED_OK`보다 엄격하게 본다.
