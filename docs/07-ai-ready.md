...
>
> 미설정 기본값은 `AI_PROVIDER=mock`이다. 실제 AI 시연은 `backend/.env`에서 `openai`로 켠다.
> 이 모드에서도 여행 없는 `RULE_CHECK`(S-09 챗봇)와 `WEIGHT_ESTIMATE`는 Mock이다.
> 각 작업의 `modelName`에는 실제 실행에 맞춰 OpenAI 모델명 또는 `mock`을 기록한다.
...
| AI-03 | `WEIGHT_ESTIMATE` | 무게를 **범위**로 추정하고 한도와 비교한다 | UC-10 | `S-06` `S-07` | 입력 물품·수량에 적용한 Mock 품목 범위 + 서버 합산 | 품목 중량 DB + LLM 보정 |
| AI-04 | `RULE_CHECK` | 질문·물품에서 **속성을 구조화**하고 판정을 **설명**한다 | UC-07 · UC-08 | `S-06` `S-08` `S-09` | 여행 연결은 실제 AI, 여행 없는 챗봇은 Mock. 판정은 모두 **규칙 엔진** | 규정 데이터 갱신 |
...
| 파일 | 하는 일 |
| --- | --- |
| `domain/ai/OpenAiClient.java` | `AiClient` 구현. 사진·추천·여행 검수는 실제 호출하고 챗봇·무게는 `MockAiClient`에 넘긴다. 응답 검증·보정·재시도 1회 |
...
혼합 모드의 사용 이력을 정확히 남기기 위해 `modelName(jobType, tripId)`도 기본 메서드로
추가했다. 실제 구현만 작업별 이름을 고르고, 기존 Mock 구현은 항상 `mock`을 반환한다.
...
```json
{
  ...
  "properties": {
    ...
    "weatherAsOf": {
      "type": ["string", "null"],
      "format": "date",
      "description": "서버가 실제 사용한 예보·계절 자료의 기준일을 넣는다. 조회 실패로 자료를 쓰지 않았으면 null"
    }
  },
  "required": [
    "items",
...
후보는 내 목록에 없으며 완료율에 영향을 주지 않는다. SQL 시드도 미채택 추천 후보를
체크리스트 행으로 미리 넣지 않는다.
...
`item_weights` 시드값을 사용하며 SQL 시드에도 사진 인식 가위가 자동 등록 상태로 들어 있다.
...
`AI_PROVIDER=openai`인 혼합 모드에서도 여행 없는 `RULE_CHECK`와 `WEIGHT_ESTIMATE`는 이
Mock을 사용한다. 따라서 발표 시 Mock 사용 범위는 **챗봇의 물품·속성 구조화/답변 문장**과
**무게의 품목별 범위**이며, 반입 판정·무게 합산은 Mock AI가 아니라 서버 규칙이다.
...
- SQL 시드는 사진 인식 8개 자동 완료·사진 미확인 여권 1개를 반영한다. 추천 후보는 채택 전이므로
  체크리스트에 미리 넣지 않는다. 빈 여행부터 시작하는 흐름은 H2·실제 OpenAI HTTP E2E로 검증한다.
...
| ERD | 현재 12개 테이블과 Entity 매핑을 H2 전체 테스트 및 PostgreSQL 17.6 `ddl-auto=validate` 기동으로 대조 |

### 사진 자동 등록·추천 승인 수용 기준 — 구현·E2E 검증 완료
...
| 같은 BAG_CHECK 완료 재처리·반복 GET | 등록은 한 번, 사용자 사후 수정·삭제를 되돌리지 않음. 새 재분석은 수정한 인식 행만 보존하고, 이름이 다른 새 결과는 순서와 무관하게 모두 저장 |
...
### OpenAI 연동 검증 — 2026-09-04

`gpt-4o-mini`와 `https://api.openai.com/v1`에 저장소의 데모 사진과 도쿄 3박4일 입력을
보냈다. `-Dai.smoke=true`가 없으면 실제 호출 테스트는 건너뛰므로 일반 빌드에는 요금이 나가지 않는다.
...
| 실제 호출 결과 | 확인한 것 |
| --- | --- |
| `BAG_CHECK` | 입력 `photoId`, 한국어 물품명, `qty` 1~99, `confidence` 0~1, `failedPhotoIds` 계약을 통과했다 |
| HTTP E2E `BAG_CHECK` | 사진 1장에서 셔츠 4·청바지 1·샌들 1·초록색 가방 1을 인식했다. 감지 4개와 `PHOTO / PREPARED` 목록 4개가 저장되고 작업 모델명은 `gpt-4o-mini`였다 |
| `PACKING_LIST` | 이미 챙긴 충전기와 미완료 우산을 다시 내지 않았고 모든 후보에 `reason`이 있다 |
...
| 날씨가 실제로 프롬프트에 들어갔다 | 도쿄 입력에서 Open-Meteo 기준일이 `weatherAsOf`에 기록됐다 |
...
앱을 띄운 HTTP 전 구간도 H2에서 검증했다. 회원가입·로그인·여행 생성·사진 업로드 후
`POST /api/ai-jobs`의 `202` 응답을 받고 폴링했으며, 실제 OpenAI 결과와 체크리스트 자동 등록을
확인했다. 프런트는 `VITE_USE_MOCK=false`에서 실제 세션 로그인·여행 목록·사진 화면까지 확인했다.

### OpenAI 키를 사용한 실제 모델 검증 — 2026-09-04 통과

Playground 검증은 PM 확인 항목으로 별도 유지한다. 애플리케이션 경로는 실제 OpenAI 스모크와
HTTP E2E를 통과했으며, Playground 실행 여부와 혼동하지 않는다.
...
| 항목 | 환경 변수 | 지금 값 | 비고 |
| --- | --- | --- | --- |
| 제공자 | `AI_PROVIDER` | 로컬 시연 `openai` · 미설정 기본 `mock` | `openai`면 사진·추천·여행 검수만 실제 호출. 챗봇·무게는 Mock |
| 모델명 | `AI_MODEL` | `gpt-4o-mini` | 이미지 입력·Chat Completions·Structured Outputs 지원 모델. Mock 작업 이력에는 `mock` 기록 |
...
| 단계 | 할 일 | 예상 난이도 | 왜 |
| --- | --- | --- | --- |
| 1 | `AI_PROVIDER` 분기와 `OpenAiClient` | 낮음 | **완료.** 사진·추천·여행 검수는 실제, 챗봇·무게는 Mock인 작업별 분기다 |
...
| 3-3 | `RULE_CHECK` — 실제 2단계 호출 | 중간 | **완료.** 여행 연결 검수는 1차 구조화 → 규칙 엔진 → 2차 설명. 여행 없는 S-09는 Mock 유지 |
| 3-4 | `WEIGHT_ESTIMATE` — LLM 보정 | 낮음 | 합산은 산식이 정본이다. LLM 은 마스터에 없는 품목의 무게를 상식으로 채우는 **보정** 역할만 한다(확장 지점 표 「나중」) |
| 4 | AI 호출을 DB 트랜잭션 밖으로 분리하고 영속 큐/복구 워커 도입 | 중간 | 커넥션 점유와 재시작 `PENDING`을 해결하되 FE의 상태 조회·폴링 계약은 유지한다 |
| 5 | `transport_rules` 갱신 잡 — 출처 재확인 날짜 자동 갱신 | 중간 | `checkedAt` 이 오래되면 판정 근거가 약해진다 |
| 6 | 비용·토큰 사용량 모니터링과 계정별 rate limit | 낮음 | `ai_jobs` 에 토큰 수를 기록하고 데모 밖의 무제한 호출을 막는다 |

## 알려진 한계

- **개정 계약은 FE 타입·Mock·자동 등록 저장 로직과 시드에 반영됐다.** H2 전체 API 테스트,
  실제 OpenAI 사진 HTTP E2E와 브라우저 세션 로그인·여행·사진 화면 연결을 통과했다.
- **로그인 필수 흐름이 구현됐다.** 세션·CSRF·가입·로그아웃·소유권·사진 보호와
  `users.login_id` SQL·시드·JPA 매핑을 사용한다.
- **이미지 4종 재생성을 완료했다.** PNG·PUML·SVG를 03의 MD 흐름·화면 상세 및 코드와 대조했다.
...
- **날씨는 공항이 있는 도시 기준이다.** `S-02` 가 공항을 드롭다운으로 받고 출발지·도착지를
  거기서 파생시키면서(#53), 사용자가 도시를 따로 고칠 수 없게 됐다. 공항과 도시가 어긋난
  상태를 없애려는 선택인데, 대신 **나리타로 들어가 요코하마에 묵는** 경우를 표현할 수 없다.
  그때 `PACKING_LIST` 의 날씨는 도쿄 기준으로 나간다. 숙소를 따로 받게 되면(03 의 선택 항목,
  현재 TBD) 그 값을 쓰는 편이 맞다.
- **날씨 원문 전체는 보관하지 않는다.** 출처·기준일은 output에 남기지만 과거 응답의 완전한
  재현에 필요한 날씨 원문 스냅샷은 실제 연동 시 검토한다.
- **여행 연결 RULE_CHECK의 지연·비용은 재지 않았다.** 두 단계라 호출이 두 번 나간다.
  S-09 챗봇은 Mock이라 이 비용이 없다.
...
- **Mock 경로는 규정표를 고쳐도 답변 문장이 따라오지 않는다.** 실제 모델 경로는 2차 설명이
  ...
  S-09 챗봇은 항상 이 경로다 — **규정을 고칠 때 픽스처 문장도 함께 본다.**
...
- **Gemini(OpenAI 호환)와 OpenAI `gpt-4o-mini` 모두 스모크 테스트를 통과했다.** 다만 사진 2장·입력 1건 수준이라
  정확도 측정은 아니다. `AI_MAX_TOKENS=4096`도 후보 40개를 다 채우면 모자랄 수 있다 —
...
- **프로세스 재시작 시 `PENDING` 작업을 다시 집어 가는 영속 워커는 없다.** 데모에서는 다시
  요청하고, 운영화할 때 상태 점유 시각·재시도 횟수와 큐/스케줄러 복구 정책을 추가한다.
- **사진은 로컬 파일과 DB를 함께 쓴다.** DB 커밋 실패와 프로세스 중단까지 포함한 완전한 원자성은
  없다. 운영화할 때 오브젝트 스토리지 업로드 후 DB 확정, 실패 파일 정리 잡과 보관 기간을 둔다.
- **사용자별 AI 호출 횟수·비용 제한은 없다.** 운영화할 때 계정별 rate limit과 토큰 사용량 기록을
  추가한다. 현재 사진 화면은 실제 AI 모드에서 OpenAI로 전송된다는 사실을 명시한다.
- **`AI_PROVIDER`는 `mock`과 `openai`만 허용한다.** 오타나 구현하지 않은 값은 조용히 Mock으로
  전환하지 않고 기동을 실패시켜 시연 공개 내용과 실제 동작이 달라지는 일을 막는다.
