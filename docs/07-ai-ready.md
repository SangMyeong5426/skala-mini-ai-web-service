...
- 인식이 끝나면 서버가 그 물품을 `items[]` 뒤에 이어 붙이고 **`ai_jobs.input_payload` 를 다시 쓴다.**
  그래야 저장된 입력이 실제로 판정한 그대로이고, 계약의 `items`↔`results`
  **개수·순서·식별값 일치**도 그대로 유지된다. **이 기록은 DB 안에만 있다** — `GET` 응답에는
  `input` 이 없다(06).
- **이미 문맥에 있는 인식 물품은 다시 붙이지 않는다.** 직전 결과를 되보내며 같은 사진을 다시
  첨부하면 서버가 스스로 중복 입력을 만들어, 모델이 정확히 답해도 중복 식별값 방어에 걸려
  전부 `ASK_AIRLINE` 이 된다 — 사용자가 확인해 준 속성까지 함께 사라진다.
...
- 사진에서 인식된 물품은 질문이 그 물품을 말하지 않아도 **규정 키워드가 붙는다.**
  ...
  **처음 붙인 턴뿐 아니라 후속 문맥에서도** 유지한다. 배터리에만 답했다고 아직 안 답한 가위의
  규정과 되묻기가 사라지면, 사용자는 답하지 않은 것을 다시 물어볼 기회를 잃는다.
  이미 확인된 속성은 픽스처의 미상 값으로 덮지 않는다.
