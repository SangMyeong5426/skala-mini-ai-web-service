...
| 호출 API | `POST /api/ai-jobs` (`BAG_CHECK`) → `202`<br>`GET /api/ai-jobs/{jobId}` → `200` **폴링**<br>`GET /api/trips/{tripId}/detections` → `200`<br>`PATCH /api/trips/{tripId}/detections/{detectionId}` → `200` **승인** |
