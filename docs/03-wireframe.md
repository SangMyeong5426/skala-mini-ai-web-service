...
| 특이 | **승인 전에는 다음 단계에 반영되지 않는다.** 미승인 개수를 항상 표시. 승인한 물품은 S-05 체크리스트에 자동 등록된다 |
...
| 주요 요소 | 카테고리 탭(서류·의류·전자기기·세면용품·의약품·현지 맞춤), 항목별 **출처 배지**(`사진에서 확인` / `AI 추천` / `직접 추가`)·**필수/권장 배지**·수량·체크박스, **준비 완료율**, `+ 항목 추가`, 여행지 주의사항 |
...
6. **S-05의 출처 배지**(`사진에서 확인` / `AI 추천` / `직접 추가`)를 반드시 그린다.
