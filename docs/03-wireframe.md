...
| S-02 | 홈 | 진행 중 여행 + 과거 여행 카드 목록 | UC-09 | **1차** |
| S-03 | 여행 등록 | 일정·지역·목적·이동수단·항공사·가방 입력 | UC-02 | **1차** |
| S-04 | **체크리스트** | AI 추천 결과 + 추가·삭제·수정·완료 | UC-05, UC-06 | **1차** ★ |
| S-05 | 짐 사진 등록 | 다중 업로드·미리보기·삭제 | UC-03 | **1차** |
| S-06 | 인식 결과 · 승인 | 물품 후보·수량·신뢰도 확인 후 승인 | UC-04 | **1차** ★ |
| S-07 | **검수 결과** | 준비 상태 + 예상 무게 + 반입 판정 통합 | UC-06, UC-07, UC-10 | **1차** ★ |
| S-08 | 무게 상세 | 품목별 기여도·제외 항목·한도 비교 | UC-10 | 2차 |
| S-09 | 반입 규정 상세 | 판정 근거·출처·추가정보 입력 | UC-07 | 2차 |
| S-10 | 챗봇 | 자연어 질의·추가 질문·근거 표시 | UC-08 | 3차 ★ |
| S-11 | 여행 기록 상세 | 과거 여행 조회·체크리스트 복사 | UC-09 | 3차 |
...
**사용자 목표:** 여행자가 여행 조건에 맞는 체크리스트를 만들고, 짐 사진을
검수해 준비 상태·예상 무게·반입 가능 여부를 확인한 뒤 결과를 저장한다.
...
- 오른쪽 회녹색은 회원의 검수 경로, 왼쪽 분홍색은 비회원의 규정 확인 경로다.
...
| 회원·비회원 | 비회원 제한을 적어 두고도 여행 등록 흐름으로 합류했다 | 회원은 여행 검수, 비회원은 S-10 반입 규정 확인으로 분리했다 |
| 입력 오류 | 여행 등록 오류에서 흐름이 종료됐다 | 입력값을 유지한 채 오류 항목 수정으로 되돌아간다 |
| 재방문 | 진행 중 여행을 어디서 이어가는지 불명확했다 | S-02에서 마지막 저장 단계(S-04~S-07)로 복귀한다 |
| 사람의 확인 | 인식 결과 수정 후 승인 과정이 한 번만 표현됐다 | 수정·제외·직접 입력·재촬영을 반복한 뒤 승인한다 |
| 결과 활용 | 결과 화면 이후 행동이 최종 저장뿐이었다 | 무게 상세, 규정 근거, 결과 수정, 최종 저장을 선택한다 |

### 핵심 경로

    회원: 로그인 → 여행 등록/재사용 → 체크리스트 확정 → 사진 등록
          → 인식 결과 승인 → 검수 결과 확인·수정 → 최종 저장

    비회원: 로그인 없이 이용 → 챗봇 질문 → 필요 정보 보완
            → 반입 판정·근거 확인

AI 호출의 `202 Accepted`와 폴링은 사용자 흐름이 아니라 시스템 동작이므로
[`06-api-spec.md`](06-api-spec.md)와 [`07-ai-ready.md`](07-ai-ready.md)에서
분리해 설명한다. 네 AI 작업이 하나의 엔드포인트와 `job_type`을 공유한다는
설계는 [ADR 0003](adr/0003-ai-job-endpoint.md)을 그대로 따른다.

> S-10 챗봇은 비회원 경로 외에도 회원이 어느 화면에서든 열 수 있는 보조
> 흐름이다. 핵심 목표인 여행가방 검수 경로를 가리지 않도록 별도 가지로 표시했다.
