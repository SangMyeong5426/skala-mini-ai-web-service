...
`localhost:5173`에서 현재 프런트엔드의 공통 헤더·카드·상태 UI를 확인한 뒤,
데모 주 경로인 `S-00`~`S-06`을 **1440px 데스크톱 웹 화면**으로 한 보드에 배치했다.
AI 화면에는 폴링·부분 로딩 상태를 함께 표시한다.

![S-00~S-06 와이어프레임](images/03-wireframe-s00-s06.png)

- 벡터 원본: [`images/03-wireframe-s00-s06.svg`](images/03-wireframe-s00-s06.svg)
...
| S-05 | **내 체크리스트 · AI 추천** | 좌측 내 목록(이름·수량 수정·삭제·직접 추가)·우측 추천 후보(개별/전체 추가), 실제 완료 체크 | UC-05, UC-06 | **1차** ★ |
| S-06 | **검수 결과** | 준비 상태 + 예상 무게 + 반입 판정 통합 | UC-06, UC-07, UC-10 | **1차** ★ |
| S-07 | 무게 상세 | 품목별 기여도·제외 항목·한도 비교 | UC-10 | 2차 |
| S-08 | 반입 규정 상세 | 판정 근거·출처·규정 원문 검색 (조회 전용) | UC-07 | 2차 |
| S-09 | 챗봇 | 자연어 질의·추가 질문·근거 표시 | UC-08 | 3차 ★ |
| S-10 | 여행 기록 상세 | 과거 여행 조회 | UC-09 | 3차 |
| S-11 | **여행 일정** | 날짜별로 항공편·숙소·관광 일정을 시간순으로 본다 | UC-02, UC-09 | 2차 |
...
내비게이션은 **화면 위쪽**에 둔다. 브랜드(랜딩) · **내 여행** · **여행 등록** 셋뿐이다.
단, `S-01`은 본문의 `+ 새 여행`과 목적이 겹치므로 헤더의 `여행 등록` 버튼을 숨긴다.

왼쪽 사이드바를 두지 않는다. 목적지가 셋이라 216px 기둥은 대부분이 빈칸이고, 랜딩만
위쪽 헤더를 쓰면 같은 서비스가 둘로 보인다. **`S-01`~`S-12` 전부에 걸리는 결정이다.**

나머지 화면은 내비게이션이 아니라 **들어가는 자리가 따로 있다.**

| 화면 | 어디서 들어가나 |
| --- | --- |
| `S-03` `S-04` `S-05` `S-06` | 여행 준비 3단계 표시줄. 여행을 고르면 그 안에서 이어진다 |
| `S-07` 무게 상세 | **진입 경로 보류.** `S-06` 요약을 눌러 들어가던 링크를 뺐다 — 등록을 마치는 자리에서 곁길을 내지 않기로 했다. 화면과 라우트(`/trips/{tripId}/weight`)는 남아 있고, 다시 열 자리는 **TBD** |
| `S-08` 재판정 | **보류(TBD).** S-08 은 `필요한 정보` 를 이름으로 알려 주지만, 그 값을 채워 **다시 판정받는 길은 아직 없다.** 챗봇의 `RULE_CHECK` 은 `tripId`·`itemId` 를 일부러 안 보내서(대화 이력 미저장, 02:154) 답이 대화창에만 남는다. 그래서 화면 문구도 *"물어볼 수 있습니다. 다만 이 판정은 바뀌지 않습니다"* 로 낮춘다 — 보완했다고 믿게 만드는 안내가 확인하러 온 화면에서 가장 위험하다 |
| `S-08` 반입 규정 상세 | **여행 등록 흐름에서 빼낸다.** 규정 검색은 준비를 마치는 일과 결이 다른 볼일이라, 마무리 직전에 옆길을 내지 않는다. 규정 질문은 **챗봇(`S-09`)** 이 받는다 — 어느 화면에서나 열리고 여행 등록 없이도 답한다. `S-06` 은 물품별 판정·근거·출처 링크까지만 보여준다 |
| `S-11` 여행 일정 · `S-12` 3D 가방 정리 | 여행 하위다. API 도 `/trips/{tripId}/itineraries`·`/packing-layout` 이다 |
| `S-10` 여행 기록 상세 | `내 여행` 목록의 지난 여행 카드 |

`S-01` 홈은 `/trips` 다. `/` 는 번호 없는 **서비스 소개(랜딩)** 이고 로그인 없이 열린다.
...
flowchart TD
  ...
  G --> H[별도 추가 추천 생성 중]
  H --> I[S-05 좌측 내 목록 / 우측 추천 후보]
  H -->|실패| I
  I --> J{필요한 추천을 담을까?}
  J -->|행 우측 추가 · 전체 추가| K[담은 것만 내 목록에 미완료 추가]
  J -->|담지 않음| L[후보 유지: 내 목록 미등록]
  K --> I
  I -->|실제로 챙김| M[완료 체크]
  M --> I
  I -->|추가 사진으로 확인| C
...
| 주요 요소 | 사용자 닉네임을 넣은 여행 준비 배너(버튼·보조 문구 없음), 진행 중 여행 카드(여행지·기간·**준비 완료율**), 체크리스트·사진 등록·반입 규정 바로가기, 과거 여행 목록, `+ 새 여행` 버튼 |
...
| 범위 | **항공 해외여행만.** 기차·버스·자차는 목록에 **보이되 눌리지 않는다** — `Codes.Transport` 는 넷을 그대로 갖고 있고, `transport_rules` 가 항공 기준으로만 채워져 있어 지금 고르게 하면 판정이 빈 답을 낸다. 확장은 그 표를 채우는 일이다 |
| 주요 요소 | **필수** 이동수단·항공사·출발/도착 공항·출발일·귀국일·목적<br>**자동** 출발지·도착지·목적지 국가 코드 — 공항에서 나온 값이라 고칠 수 없다. 날씨·추천이 이 도시로 돌아간다<br>**선택** 경유지·숙소·가방 크기·동행인·개인 특이사항. 경유지·숙소·동행인의 개별 API·저장 규약은 TBD |
| 국가 코드 | `countryCode`(ISO 영문 2자리)는 **필수로 보내되 묻지 않는다.** 도착 공항을 고르면 정해져 있어서, 또 물으면 두 값이 어긋난 상태가 생긴다. 목록에 없는 코드라 나라를 못 찾으면 저장을 막고 공항을 다시 고르게 한다 |
| 호출 API | `POST /api/trips` → `201 Created` + `Location` |
| 로딩 상태 | 저장 버튼 비활성 + 스피너 |
| 빈 상태 | — |
| 오류 상태 | 필드별 인라인 오류 — **칸을 고치면 그 칸의 오류만 사라진다.** **귀국일 < 출발일** 과 **지난 출발일** 은 저장 전에 칸에서 잡는다 |
...
| 주요 요소 | 드래그&드롭 / 파일 선택(다중), 미리보기 썸네일·삭제, **기내용·위탁용 구분**, 촬영 가이드(*"물건을 펼쳐놓고 찍어 주세요"*), 실제 AI 모드의 OpenAI 전송 안내, `분석 시작` |
| 호출 API | `POST /api/trips/{tripId}/photos` → `201` |
| 로딩 상태 | 파일별 업로드 진행률 |
| 빈 상태 | 점선 박스 + *"싸 놓은 짐을 찍어 올려 주세요"* |
| 오류 상태 | 형식·용량 초과 시 해당 썸네일에 표시하고 **나머지는 그대로 진행** |
| 특이 | **사진이 필수다.** 한 장도 없으면 다음으로 갈 수 없고, 버튼 옆에 그 이유를 적는다. 이 서비스의 약속이 *"사진 한 장으로"* 라 건너뛰면 여행 조건만 보고 만든 일반 체크리스트가 남는다 |
...
| 주요 요소 | 사진·인식 태그, 이름·수량·신뢰도, `자동 등록됨` 표시, `수정 저장` / `목록에서 삭제`, `확인 필요` 묶음. 사진 승인 버튼은 두지 않는다<br>**카드 안쪽 아래**: 좌측 `이전: 짐 사진`, 우측 `다음 — 체크리스트`. `다시 분석`은 이 화면에 머무는 동작이라 상단에 남긴다 |
...
| 로딩 상태 | `사진을 분석하고 목록에 등록하는 중`. **이 동안 인식 목록은 감춘다** — 지난 결과가 진행 표시 아래에 남아 있으면 다 된 것으로 읽힌다. 화면을 벗어나도 서버가 등록까지 처리한다 |
...
| 사후 수정 | 이름·수량·연결을 변경하면 같은 항목을 갱신한다. 재분석은 수정한 인식 행만 보존하고 같은 사진의 나머지 결과는 갱신한다. 여러 사진의 동일 물품 수량은 단순 합산하지 않는다. 자동 등록·중복 처리·사용자 수정 보존은 06을 따른다 |
...
| 주요 요소 | **좌측 내 체크리스트:** 카테고리·출처·필수/권장 배지·수량·실제 챙김 완료 체크·완료율·직접 추가·**항목별 이름/수량 수정**·삭제<br>**우측 AI 추천:** 후보별 이름·수량·이유·필수/권장과 **행 우측의 `추가`(개별)·머리말 우측의 `전체 추가 n개`**. 추가한 후보는 `추가됨` 표시로 재등록 방지 |
| 배치 | **좌우 2단이다.** 두 목록은 *무엇이 빠졌나*를 견주라고 있는 것이라 나란히 둔다. 쌓으면 추천이 화면 밖으로 밀려 스크롤 왕복이 된다. 1100px 미만에서는 위아래로 되돌린다 |
| 채택 방식 | **고르기 단계가 없다.** 행 우측 `추가`가 곧 채택이고, 머리말 우측 `전체 추가`가 남은 후보를 한 번에 담는다. 담는 동작은 모두 **우측**에 모으고, 좌측 체크박스는 내 목록의 **실제 챙김 완료** 하나만 쓴다. 일부 실패 시 성공분은 유지하고 실패 개수만 알린다 |
...
| **로딩 상태** | 우측에 **"추가 준비물을 추천하는 중"** 표시. 좌측 내 목록은 계속 보이며 추천 생성만으로 완료율이 바뀌지 않는다 |
...
| 필수 후보 경고 | `unacceptedRequiredCount > 0`이면 완료율과 함께 **`미채택 필수 후보 n건`**·`확인하기`를 표시한다. 누르면 우측 추천 영역의 필수 후보로 이동한다. 여권 등 RULE 후보도 포함하며 완료율 100%여도 경고를 유지한다. `null`이면 `필수 추천 확인 전`을 표시한다 |
...
| 주요 요소 | **① 준비 상태** — 완료율 막대 아래 **좌측 `챙김 완료` · 우측 `아직 안 챙김` 2단**(안 챙긴 것이 없으면 우측 가운데에 `다 챙기셨습니다`), 각 항목에 사진 비교 배지(`확인됨` / `확인 필요` / `사진에서 미확인`)<br>**② 예상 무게** — 실제 완료 항목의 `최소–대표–최대` 범위, 신뢰도·한도 대비 상태·계산 제외 이유<br>**③ 반입 판정** — 물품별 상태 배지 + 근거 한 줄<br>**카드 안쪽 아래 오른쪽**의 `최종 저장` 버튼 |
| 배치 | 세 카드는 서로 **떨어뜨려** 다른 이야기임을 보인다. ①의 두 묶음은 좌우로 나눈다 — 쌓으면 챙긴 것이 길어질수록 **정작 볼 `아직 안 챙김`이 화면 밖으로 밀린다**. 900px 미만에서는 위아래로 되돌린다 |
...
| 항목 | 내용 |
| --- | --- |
| 목적 | 판정 근거와 출처를 확인하고 규정 원문을 찾아본다 |
| 범위 | **조회 전용이다.** 값을 채워 다시 판정받는 입력칸을 두지 않는다 |
| 주요 요소 | 물품명·판정 배지, **판단 근거 문구**, **출처 링크와 확인 날짜**, 부족한 정보의 **이름**, 규정 원문 검색 |
| 재판정 | **보류(TBD).** 부족한 정보(용량·Wh·날 길이)를 채워 다시 판정받는 길이 아직 없다. 챗봇의 `RULE_CHECK` 은 `tripId`·`itemId` 를 보내지 않아(대화 이력 미저장, [`02-use-case.md`](02-use-case.md) 154행) 답이 대화창에만 남고 이 여행의 `item_rule_checks` 는 바뀌지 않는다. 그래서 화면도 *"물어볼 수 있습니다. 다만 이 판정은 바뀌지 않습니다"* 까지만 말한다 |
...
| --- | --- |
| 목적 | 지난 여행이 어땠는지 본다 |
| 범위 | **조회 전용이다.** 이 화면에서 새 여행으로 옮겨 가는 것은 없다 |
| 주요 요소 | 여행 요약, 최종 체크리스트, **사용하지 않은 물건**, `새 여행 만들기`(빈 폼으로 이동) |
| 호출 API | `GET /api/trips/{tripId}` → `200` |
| 로딩 상태 | 스켈레톤 |
| 빈 상태 | *"기록이 없습니다"* |
| 오류 상태 | *"불러오지 못했습니다"* + `다시 시도` |
| 재사용 | **복사하지 않는다.** 지난 여행을 통째로 베끼면 날짜도 목적지도 다른 여행에 옛 목록이 딸려 와, 사용자가 지우는 일부터 하게 된다. 대신 **UC-05 추천이 지난 이력을 보고** 그때 챙겼던 것을 권한다 — 지우는 일이 아니라 고르는 일이 된다. 그 입력은 [`07-ai-ready.md`](07-ai-ready.md)의 `PACKING_LIST` 스키마에 아직 없어 **보류(TBD)** 다 |
| 특이 | **과거 반입 판정을 그대로 쓰지 않는다.** 규정은 바뀌므로 새 여행에서 다시 판정한다 |
...
6. **S-05의 두 목록을 구분한다.** 좌측은 내 목록, 우측은 후보다. **체크박스는 좌측
   하나뿐이고 뜻도 하나다 — 실제 챙김 완료.** 후보를 담는 것은 우측 버튼이 맡는다.
   같은 모양(체크박스)에 다른 뜻을 두지 않는다. 출처 배지는 최초 등록 경로를 표시한다.
