...
- [ ] **화면 구현 1차** — `S-01`~`S-07` 데모 주 경로 (Mock API 호출 → 데이터 렌더링)
- [ ] 화면 구현 2차 — `S-08` 무게 상세 · `S-09` 반입 규정 상세
- [ ] 화면 구현 3차 — `S-10` 챗봇 · `S-11` 여행 기록 상세
- [ ] End-to-End 흐름 검증

> 팀 결정으로 **화면 11개 전체**를 만든다. 다만 **1차 7개가 끊김 없이 도는 것**이
