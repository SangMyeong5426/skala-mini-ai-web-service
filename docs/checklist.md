...
- [ ] 완료 작업 재처리·일부 사진 실패·사용자 수정 보존·낮은 신뢰도 자동 등록 수용 기준 실행
...
- [ ] FE 타입·S-04~S-06와 BE 자동 등록·추천 채택·무게 Mock에 개정 계약 반영
- [ ] 기존 시드를 개정 상태로 갱신 — 사진 인식 가위 등록과 추천 채택 시점 구분
- [ ] 07의 사진 자동 등록·추천 승인 수용 기준 실행 — 빈 목록, 선택분만 추가, 재승인, 완료 후 무게 재계산
