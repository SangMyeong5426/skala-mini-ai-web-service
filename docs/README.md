...
| DB 테이블·컬럼 | 기존 10개 테이블 유지. 후보·채택 연결은 기존 `ai_jobs.output_payload`의 서버 필드 사용 |
| 다이어그램 | 03의 MD Mermaid가 최신 흐름. 기존 PNG·PUML·SVG의 명칭·승인 단계·책임 주석 재생성은 TBD |
| 실제 기능 구현 | 후속 반영 필요. BE는 설정 뼈대, FE는 화면 자리·공통 타입·폴링과 브라우저 Mock이 있으며 개정 API 구현 완료를 뜻하지 않음 |
