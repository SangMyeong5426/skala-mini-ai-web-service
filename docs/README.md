...
| DB 테이블·컬럼 | **12개 테이블.** 기존 10개 + `trip_itineraries`(일정·캘린더) · `item_placements`(3D 가방 정리) — 이 둘은 `schema.sql`에 **적용 완료**다. 목표 ERD의 `users.login_id`는 **SQL·시드·DB 미반영**이라 성격이 다르다. 후보·채택 연결은 기존 JSON 필드 사용 |
| 다이어그램 | 4종 PNG·PUML·SVG를 로그인 포함 목표 설계로 재생성. S-00 진입·인증/소유권·로그인 아이디와 사진 자동 등록·추천 선택 흐름 반영. `05-erd`는 새 표 둘까지 포함해 다시 렌더했다 |
| 실제 기능 구현 | **BE 는 도메인 API 30개를 구현했다**(테이블 12개 · 엔터티 · Mock AI · 폴링). 다만 **인증과 사진 자동 등록은 아직 코드에 없다** — 로그인은 시드 사용자 고정이고 사진은 승인 게이트를 거친다. 이 둘을 개정 계약에 맞추는 것이 다음 작업이다. FE는 화면 자리·공통 타입·브라우저 Mock 단계다 |
