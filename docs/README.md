...
아이디·비밀번호다. S-00 인증 화면을 더해 총 11개 화면, 일정·정리까지 총 30개 JSON API를 설계한다.
AI는 미설정 시 Mock이다. 로컬 `AI_PROVIDER=openai`에서는 `BAG_CHECK`·`PACKING_LIST`·여행 연결 `RULE_CHECK`가 실제 모델을 부르고, 여행 없는 S-09 챗봇과 `WEIGHT_ESTIMATE`는 Mock이다([07](07-ai-ready.md#모델-파라미터)). 사진 자동 등록은 BAG_CHECK 완료 처리에 포함하고 S-04 PATCH는 선택적 사후 수정이다.
...
| 실제 기능 구현 | **BE 는 API 30개를 전부 구현했다** — 업무 26개 + 인증 4개. FE는 S-00~S-06의 백엔드 호출·AI 폴링·사진 자동 등록 후 추천 연결을 구현했다. OpenAI 사진·추천·여행 검수 스모크와 H2 자동 등록 E2E를 통과했다 |
| 기존 시드 | 사진 인식 8개 자동 완료·사진 미확인 여권 1개로 갱신. 미채택 추천은 체크리스트에 미리 넣지 않음 |

프런트 E2E 시연은 `frontend/.env`의 `VITE_USE_MOCK=false`를 사용한다. 실제 OpenAI 호출과
작업별 모델명 저장 결과는 사진 업로드 → 폴링 → 자동 등록 → 추천 후보 표시 순서로 확인한다.
