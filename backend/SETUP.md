...
| `AI_PROVIDER` | `mock` | `openai` 로 바꿔야만 실제 호출이 나간다. 그때도 `BAG_CHECK`(사진 인식) 하나뿐이고 나머지 3종은 Mock 이다. 구현이 없는 값(`anthropic`)은 조용히 mock 으로 떨어진다 |
...
| `AI_TIMEOUT_MS` · `AI_BASE_URL` | `60000` · `https://api.openai.com/v1` | 호출 제한 시간과 API 주소. 주소는 Azure·프록시를 쓸 때만 바꾼다 |
...
- `BAG_CHECK` 는 `AI_PROVIDER=openai` 에서 실제 OpenAI 를 부른다(`OpenAiClient`). **SDK 는 넣지 않았고**
  `RestClient` 로 직접 부른다. 큐는 아직 없다 — 나머지 3종도 그때 Mock 그대로다.
