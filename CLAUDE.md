...
- **AI는 미설정 시 Mock이다.** `AI_PROVIDER=mock`이 애플리케이션 기본값이다. 실제 AI 시연은
  `backend/.env`에서 `openai`로 명시한다. Mock이든 실제든 응답은
  `docs/07-ai-ready.md`의 스키마를 **정확히** 지킨다.
- **실제 모델을 부르는 곳은 셋이다.** `AI_PROVIDER=openai`일 때만 나간다.
  ...
  - 여행에 연결된 `RULE_CHECK` — **모델을 두 번 부른다.** 1차로 물품·속성을 구조화하고,
    2차로 판정을 설명한다. 그 사이에 규칙 엔진이 들어간다.
  - 여행 없는 `RULE_CHECK` — S-09 챗봇이며 팀 결정대로 **Mock을 유지한다.**
- **판정과 합산은 AI가 아니다.** 반입 판정은 규칙 엔진(`transport_rules`)이, 무게 합산은
  산식이 정본이다. AI는 `RULE_CHECK`의 속성 구조화·이유 설명과 `WEIGHT_ESTIMATE`의 무게
  보정만 맡는다. **이 경계를 지운 채 AI에게 판정을 시키지 않는다.**
  `WEIGHT_ESTIMATE`는 아직 Mock이다.
- **모델이 낸 값을 그대로 믿지 않는다.** 스키마를 강제해도 값까지 맞다는 뜻은 아니다.
  범위를 벗어난 수, 입력에 없는 식별값, **사용자가 말하지 않은 속성**은 서버가 버린다.
  특히 배터리 정격(Wh)처럼 판정을 가르는 값은 사용자가 확인해 준 것만 쓴다.
- **`openai`는 구현에서 쓰는 프로토콜 이름이다.** 실제로 어디로 나갈지는 `AI_BASE_URL`이 정한다.
  현재 실제 시연은 OpenAI의 `https://api.openai.com/v1`을 사용한다. 키는 `backend/.env`에만 둔다.
- **여기서 더 늘리지 않는다.** LLM SDK를 설치하지 않는다 — `RestClient`로 직접 부른다.
  새 `jobType`을 만들지 않고, 위 셋 밖의 자리에 AI를 붙이지 않는다.
